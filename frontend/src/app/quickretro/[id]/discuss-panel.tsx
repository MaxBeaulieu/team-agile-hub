...
function DiscussionGroup({
  ...
  const otherNotes = group.cards.filter(
  ...
  return (
    ...
    >
      <div className="px-4 py-3 space-y-2">
        ...
        <CardActionItems items={groupActionItems} className="pt-1" />
      ...
      <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
        <div
          ...
          style={{
            width:
              totalGroups > 0
                ? `${(discussedCount / totalGroups) * 100}%`
                : "0%",
          }}
        />
      </div>
      ...
      <div className="space-y-3">
        {sorted.map((group) => (
          <DiscussionGroup
            ...
            session={session}
            groupActionItems={group.cards.flatMap(
              (c) => itemsByCard[c.id] ?? [],
            )}
            onRefresh={onRefresh}
