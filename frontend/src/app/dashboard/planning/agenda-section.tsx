...
import type {
  RecurringAgendaItemData, TalkingPointData, TalkingPointNoteData,
  SprintDetail, TeamMemberData,
} from './page'

interface Props {
  items: RecurringAgendaItemData[]
  teamId: string
  sprint: SprintDetail
...
function AgendaRow({ item, sprint, teamId, teamMembers, onUpdate }: {
  item: RecurringAgendaItemData
  sprint: SprintDetail
  teamId: string
  teamMembers: TeamMemberData[]
  onUpdate: () => void
  ...
  const points = [...(item.talking_points ?? [])].sort((a, b) => a.order - b.order)
  ...
  function saveTitle(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await api.patch(`/api/teams/${teamId}/recurring-agenda/${item.id}`, { title: title.trim() })
  ...
  function deleteItem() {
    startTransition(async () => {
      try {
        await api.delete(`/api/teams/${teamId}/recurring-agenda/${item.id}`)
        onUpdate()
      } catch (err) {
  ...
  function addPoint(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await api.post(`/api/teams/${teamId}/agenda/${item.id}/talking-points`, {
  ...
  return (
    <div className="py-2 group/item border-b border-border last:border-0">
        ...
        {editingTitle ? (
          <form onSubmit={saveTitle} className="flex-1 flex gap-1">
            ...
            <Button type="button" variant="ghost" size="sm" className="h-7 text-xs px-1"
              onClick={() => { setTitle(item.title); setEditingTitle(false) }}>✕</Button>
          </form>
        ) : (
          <>
            <div
            ...
            >
              ...
              {item.lastStatus && (
                <span className="ml-2 text-[10px] bg-accent text-muted-foreground px-1.5 py-0.5 rounded-full">
                  {item.lastStatus}
                </span>
              )}
...
export function AgendaSection({ items, teamId, sprint, teamMembers, onUpdate }: Props) {
  ...
  function addItem(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await api.post(`/api/teams/${teamId}/recurring-agenda`, { title: newTitle.trim() })
        setNewTitle('')
        setAddOpen(false)
        onUpdate()
      } catch (err) {
      ...
      <div className="rounded-xl border border-border bg-card px-4 py-1">
        {items.length === 0 && !addOpen && (
          <p className="py-4 text-xs text-muted-foreground text-center">
            No recurring agenda items. Add things that should be revisited every sprint.
          </p>
        )}
        ...
        {addOpen && (
          <form onSubmit={addItem} className="flex gap-2 border-t border-border pt-3 pb-3">
            <input
              autoFocus
              value={newTitle}
              onChange={(e) => setNewTitle(e.target.value)}
              placeholder="e.g. Review blockers, Update roadmap…"
            ...
            <Button type="button" variant="ghost" size="sm" className="h-8 text-xs px-2"
              onClick={() => { setNewTitle(''); setAddOpen(false) }}>Cancel</Button>
          </form>
        )}
      </div>
    </section>
