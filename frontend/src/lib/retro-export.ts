...
export function buildRetroMarkdown({
  ...
  const nameByUserId = new Map(
    teamMembers.map((m) => [m.userId, m.displayName]),
  );
  ...
  if (actionItems.length === 0) {
    ...
    for (const item of actionItems) {
      const assignee = item.assigneeId
        ? nameByUserId.get(item.assigneeId)
        : null;
      ...
      lines.push(
        `- ${checkbox} ${escapeInline(item.text)} — _(${meta.join(", ")})_`,
      );

      const card = item.retroCardId ? cardById.get(item.retroCardId) : null;
      if (card)
        lines.push(
          `  - From **${escapeInline(card.column)}**: ${escapeInline(card.content)}`,
        );
    }
    lines.push("");
  }
  ...
  if (groups.length === 0) {
    ...
    for (const group of groups) {
      ...
      const heading =
        group.isGroup && group.label ? group.label : anchor.content;
      const status = group.cards.every((c) => c.isDiscussed)
        ? "discussed"
        : "not discussed";
