...
function MoodSummary({ moodCheckins, label }: { moodCheckins: MoodCheckin[], label: string }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{label === 'entry' ? 'Entry Mood' : 'Exit Mood'}</p>
      <div className="flex items-end gap-2 h-12">
        {dist.map(m => (
          <div key={m.value} className="flex flex-col items-center gap-1 flex-1">
            <span className="text-[10px] text-muted-foreground tabular-nums">{m.count > 0 ? m.count : ''}</span>
            <div
...
export function WrapUpPanel({
  session, sprintName, cards, moodCheckins, teamMembers, actionItems, roster,
  currentUserId, teamId, isFacilitator, onRefresh,
}: Props) {
  ...
  async function pickExitMood(value: number) {
    ...
    setSaving(true)
    try {
      await api.post(`/api/teams/${teamId}/retro/${session.id}/mood`, {
        exitMood: value,
      })
      onRefresh()
    } catch (err) {
  ...
  const actionGroups = actionItems.reduce<ActionItemGroup[]>((acc, item) => {
    const card     = item.retroCardId ? cardById.get(item.retroCardId) ?? null : null
  ...
  function exportMarkdown() {
    ...
    const members = [...teamMembers, ...roster].map(m => ({ userId: m.userId, displayName: m.displayName }))
    const visibleMoodCheckins = moodCheckins.filter(m => rosterUserIds.has(m.userId))
    ...
    <div className="flex flex-col gap-8 p-8 max-w-xl mx-auto w-full">
      ...
      {!isCompleted && (
        <div className="space-y-3">
          <p className="text-sm font-medium text-center">How are you feeling after this retro?</p>
            ...
            >
              ...
              <div className="shrink-0 border-t border-border pt-2 @sm:w-40 @sm:border-t-0 @sm:border-l @sm:pl-3 @sm:pt-0">
                {group.card ? (
                  ...
                  <p className="text-[11px] italic text-muted-foreground">Not linked to a card</p>
                )}
              </div>
            </div>
          ))}
      ...
      {isFacilitator && !isCompleted && (
        <p className="text-xs text-muted-foreground text-center">
          Use the <strong>Complete Retro</strong> button in the header to finish the session.
        </p>
      )}
    </div>
  )
